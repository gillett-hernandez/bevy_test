...
use crate::{config::GameConfig, mods::engines::SubmarineEngine, physics::Physics};
...
pub fn vertical_bound_system(
    ...
    for (_e, mut hp, mut physics, mut transform, maybe_submarine) in query.iter_mut() {
        if transform.translation.y > game_config.upper_bound {
            ...
            if let Some(hp) = hp.as_mut() {
                hp.hp -= game_config.upper_hp_drain;
            }
            ...
            if maybe_submarine.is_none() {
                if let Some(hp) = hp.as_mut() {
                    hp.hp -= game_config.lower_hp_drain;
                }
            }
        } else {
            continue;
