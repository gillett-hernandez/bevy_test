...
pub fn player_bullet_collision_system(
    ...
    mut query1: Query<(Entity, &mut HP, &Transform), With<Player>>,
    query2: Query<(Entity, &Bullet, &Transform)>,
    // debug_timer: Res<DebugTimer>,
) {
    let mut counter = 0;
    let (_, mut hp, player_tx) = query1.single_mut();
    for (bullet_entity, bullet, bullet_tx) in query2.iter() {
        if !bullet.hostile {
...
pub fn enemy_bullet_collision_system(
    mut commands: Commands,
    mut query1: Query<(Entity, &mut HP, &Transform), With<Enemy>>,
    mut query2: Query<(Entity, &mut Bullet, &Transform)>,
    // debug_timer: Res<DebugTimer>,
) {
    let mut counter = 0;
    for (_enemy_entity, mut hp, enemy_tx) in query1.iter_mut() {
        for (bullet_entity, mut bullet, bullet_tx) in query2.iter_mut() {
            if bullet.hostile {
